...
const move_survie = function(req, res, query, bfld, wave, oppo, heros, niveau, life_enemy){
    ...
    if (play === "Haut"){
        ...
        // epee niveau 1
	if (heros[0].epee > 0 && query.arme === "epee") {
			if(perso.scry === 1){
		...
		// lance	
	}else if(heros[0].lance > 0 && query.arme === "lance") {
		if(perso.scry === 1 || perso.scry === 0){
            for(let k = 0; k < oppo.length; k++){
                tmp = oppo[k];
                if (tmp.x === perso.x && tmp.y === perso.y + 1 && tmp.x === perso.x && tmp.y === perso.y - 1){
                    target = tmp;
                    checktarget = checktarget + 1;
                }else if (tmp.x === perso.x && tmp.y === perso.y + 1 || tmp.x === perso.x && tmp.y === perso.y - 1) {
            		target = tmp;
                    checktarget = checktarget + 1;
			}
                if (checktarget === 1) {
...
//			choix = choix_arme(req, res, query, bfld, wave, oppo, heros, niveau, life_enemy, page, marqueurs, reponse);
